...
// Shelved hero segment hover (background swap + custom cursor) — restore later:
// import { useCallback, useState } from "react";
// import { getHighlightBackgroundUrl } from "@/content/hero-highlights";
...
// Shelved hover timing constants
...
export function HeroStage({ highlights }: HeroStageProps) {
  // Shelved: segment hover drives background + custom cursor label
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      for (let attempt = 0; attempt < 30; attempt += 1) {
        ...
        if (
          flowEl?.querySelector("[data-hero-load-word]") ||
  ...
  // Shelved: fade portrait when a highlight is hovered
  // useEffect(() => {
  ...
  useEffect(() => {
    const portrait = portraitRef.current;
    if (!portrait) return;
    gsap.set(portrait, { opacity: 0.45 });
  }, [isMobileHero]);

  // Shelved: setActiveId on highlight hover
  ...
  return (
    <>
      {/* Shelved: highlight hover background crossfade
      {!isMobileHero ? (
        <HeroBackground
      ...
      */}
      ...
      <div
        className={`body-wrapper${
          // Shelved: cursorEnabled && !isMobileHero ? " body-wrapper--custom-cursor" : ""
          ""
        }${
          // Shelved: activeId && !isMobileHero ? " body-wrapper--item-active" : ""
          ""
